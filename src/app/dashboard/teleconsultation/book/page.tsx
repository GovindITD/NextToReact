...
export default function BookTeleconsultationCenterSelectionPage() {
  const router = useRouter();
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-bold font-headline text-primary">Book a Teleconsultation</h1>
        <p className="text-muted-foreground">Please select a hospital center to proceed.</p>
      </div>
      <div className="max-w-2xl mx-auto space-y-4">
        {centers.map((center) => (
          <Link href={`/dashboard/teleconsultation?center=${encodeURIComponent(center.name)}`} key={center.name}>
